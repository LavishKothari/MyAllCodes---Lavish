...
class Rod
{
	...
	Graphics g;
	static double angle=Math.PI/180.0;
		...
		while(true)
		{
			...
			for(int i=0;i<lst.size();i++)
				for(int j=i;j<lst.size();j++)
					rotate(lst.get(i).a,lst.get(i).b,lst.get(j));
		}
		
	}
	public static void rotate(double x,double y,Rod r)
	{	
		Rod rod=new Rod();
		rod.a=(r.a-x)*Math.cos(r.angle)-(r.b-y)*Math.sin(r.angle)+x;
		rod.b=(r.a-x)*Math.sin(r.angle)+(r.b-y)*Math.cos(r.angle)+y;
		
		rod.c=(r.c-x)*Math.cos(r.angle)-(r.d-y)*Math.sin(r.angle)+x;
		rod.d=(r.c-x)*Math.sin(r.angle)+(r.d-y)*Math.cos(r.angle)+y;
