/*
    program for pair wise swapping.
*/
#include<stdio.h>
#include<stdlib.h>
struct Node
...
struct Node*reverseListInGroup(struct Node*start,int n)
{
    int size=listSize(start),i;
    for(i=1;i+n-1<size;i+=n)
        start=reverseListAdvanced(start,i,i+n-1);
    start=reverseListAdvanced(start,i,size);
    return start;
}
struct Node*pairWiseSwapping(struct Node* start)
{
    return reverseListInGroup(start,2);
}
int main()
{
